...
dt <- sample_n(happiness_dt, 100) # dplyr package
...
# add density plot (google: density plot, ggplot2)
...
### distribution of social score (independent variable) not normal (left skewed)
...
# standardize (dt$country - mean(dt$country))/sd(dt$country) -> check Z > 3.5
...
dt1 <- dt
#2. subsetting
dt1 <- dt1 %>% filter(Corruption != min(Corruption)) # from dplyr package
# dt1 <- dt1[Corruption != min(Corruption),] # equivalent
...
# Full model
linear_reg1 <- lm(Ladder ~ LogGDP + Social + HLE + Freedom + 
                  Generosity + Corruption + Positive + Negative + gini, 
                  data = original_dt)
summary(linear_reg1) # R square: the proportion of variation which can be explained by the model 77.9%
qqnorm(linear_reg1$residuals); qqline(linear_reg1$residuals)
shapiro.test(linear_reg1$residuals) # Shapiro-Wilk test -> fail to reject null hypothesis -> normal
# Nest model
linear_reg2 <- lm(Ladder ~ LogGDP + Social + HLE + Freedom + Positive, 
                  data = original_dt)
summary(linear_reg2) # R square: 77.4%
qqnorm(linear_reg2$residuals); qqline(linear_reg2$residuals)
shapiro.test(linear_reg2$residuals)
# Likelihood Ratio Test
lrtest(linear_reg1, linear_reg2) # from lmtest package -> no significant difference btw two models
# -> prefer the nest model
...
norm_mardia <- mvn(data = indep_dt, mvnTest = "mardia",multivariatePlot = "qq"); norm_mardia # not normal
norm_hz <- mvn(data = indep_dt, mvnTest = "hz"); norm_hz # same result

# Check normality by using 'RVAideMemoire' package
mshapiro.test(indep_dt) # not normal

# Box-Cox Transformation
# make all values to be positive
trans_dt <- original_dt # 10 variable make a copy
trans_dt[,2:10] <- trans_dt[,2:10] + 1 # make them into positive number
for (i in 2:10) {
  bc <- boxcox(trans_dt[,i] ~ 1)
  lambda <- bc$x[which.max(bc$y)]
  trans_dt[,i] <- (trans_dt[,i]^lambda-1)/lambda
}
mshapiro.test(trans_dt) # still not normal
...
linear_reg3 <- lm(Ladder ~ LogGDP + Social + HLE + Freedom + Generosity + Corruption + Positive + Negative + gini, 
                  data = trans_dt)
summary(linear_reg3)
summary(linear_reg1) # compare to original data regression model
...
vif(linear_reg1) # check if greater than 5
cor_tbl <- cor(original_dt)
cor_tbl >= 0.8 ## No Multicollinearity

## Check Heterogeneity (standardized fitted value plot) not constant variance


#' --------------------------------------------------------------------------------------------------------------------------

# Multiple Linear Model using Principal Components
n = 100 # should be changed if removed observation
...
summary(PC.model.2)  # R square is the same as the original data full model

lrtest(PC.model,PC.model.2) # has difference
...
# scree plot for standardized PCs 
...
# If we want to get 90 % variation -> take 6 PCs
...
summary(PC.model.new.1)  # W2 and W6,7,8,9 seem not to be significant R square: 77.9%

PC.model.new.2 = lm(original_dt$Ladder ~  W.new[,1] + W.new[,3] + W.new[,4] + W.new[,5])
summary(PC.model.new.2) # 78.4% better than full model

# lrt test
lrtest(PC.model.new.1,PC.model.new.2) # no difference -> prefer nest model
